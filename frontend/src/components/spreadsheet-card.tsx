...
export function SpreadsheetCard() {
  ...
  async function create() {
    ...
    try {
      ...
      const data = (await res.json()) as { spreadsheet_id: string; url: string };
  ...
  return (
    <section className="settings-card" aria-labelledby="sheet-title">
      ...
      {connected && hasSheet && status?.spreadsheet_url && (
        <a className="button" href={status.spreadsheet_url} target="_blank" rel="noreferrer">Open spreadsheet ↗</a>
      )}
